...
public class Toxic implements Enchantment, Listener {
    ...
    public void action(@Nullable Location location, @Nullable LittlePlayer summoner, LittlePlayer victim, int lvl, double damage) {
        switch (lvl) {
            case 1 -> victim.addEffect(new PotionEffect(PotionEffectType.POISON, 20, 0));
            case 2 -> victim.addEffect(new PotionEffect(PotionEffectType.POISON, 40, 1));
            case 3 -> victim.addEffect(new PotionEffect(PotionEffectType.POISON, 60, 2));
        }
    }
}
