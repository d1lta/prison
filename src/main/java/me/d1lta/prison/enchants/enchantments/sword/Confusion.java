...
public class Confusion implements Enchantment, Listener {
    ...
    public void action(@Nullable Location location, @Nullable LittlePlayer summoner, LittlePlayer victim, int lvl, double damage) {
        switch (lvl) {
            case 1 -> victim.addEffect(new PotionEffect(PotionEffectType.CONFUSION, 80, 0));
            case 2 -> victim.addEffect(new PotionEffect(PotionEffectType.CONFUSION, 90, 0));
            case 3 -> victim.addEffect(new PotionEffect(PotionEffectType.CONFUSION, 110, 0));
        }
    }
}
