...
public class ScoreboardP {
    ...
    public void scoreboard(LittlePlayer pl) {
        ...
        Objective objective = scoreboard.registerNewObjective("Test", "Dummy", DComponent.create("Prison Classic"));
        objective.setDisplaySlot(DisplaySlot.SIDEBAR);

        Team blocks = scoreboard.registerNewTeam("blocks");
        blocks.addEntry("Вскопано блоков » §b");
        blocks.suffix(
                DComponent.create(NumberUtils.withK(pl.getBlocks()), TextColor.color(255, 170, 0)));

        Team money = scoreboard.registerNewTeam("money");
        money.addEntry("Баланс » §b");
        money.suffix(
                DComponent.create(String.format("%.2f", pl.getMoney()) + "$", TextColor.color(85,255,85)));

        Team level = scoreboard.registerNewTeam("level");
        level.addEntry("Уровень » §b");
        level.suffix(DComponent.create(String.valueOf(pl.getLevel()), TextColor.color(255, 255, 85)));

        Team rats = scoreboard.registerNewTeam("rats");
        rats.addEntry("Крыс убито » §b");
        rats.suffix(DComponent.create(String.valueOf(pl.getRats())));

        Team kills = scoreboard.registerNewTeam("kills");
        kills.addEntry("Убийств » §b");
        kills.suffix(DComponent.create(String.valueOf(pl.getKills())));

        Team deaths = scoreboard.registerNewTeam("deaths");
        deaths.addEntry("Смертей » §b");
        deaths.suffix(DComponent.create(String.valueOf(pl.getDeaths())));

        Team online = scoreboard.registerNewTeam("online");
        online.addEntry("Онлайн » §b");
        online.suffix(DComponent.create(String.valueOf(Bukkit.getServer().getOnlinePlayers().size())));
        ...
        faction.suffix(DComponent.create(pl.getFaction().getName(), color));
